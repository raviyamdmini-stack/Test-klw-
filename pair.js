...
case 'weekly': {
    ...
    const sortedStats = Object.entries(rankDb)
        .map(([id, data]) => {
            ...
            if (mode === 'daily' && data.daily?.dayKey === currentDay) count = data.daily.count;
            if (mode === 'weekly' && data.weekly?.weekKey === currentWeek) count = data.weekly.count;
            return { id, count };
        })
        .filter(u => u.count > 0)
    ...
    topList.forEach((u, i) => {
        ...
        mentions.push(u.id);
        mentionText += `${medal} @${u.id.split('@')[0]} : *${u.count}*\n`;
    });
...
case 'myrank': {
    ...
    if (!userStats) {
        return AlexaInc.sendMessage(chatId, {
            text: '📉 You have no rank yet. Start chatting!'
        }, { quoted: msg });
    }
    ...
    const myIndex = sortedUsers.findIndex(u => u.id === senderId);
    const myRank = myIndex + 1;

    const daily = userStats.daily?.dayKey === getDayKey() ? userStats.daily.count : 0;
    const weekly = userStats.weekly?.weekKey === getWeekKey() ? userStats.weekly.count : 0;

    let medal = myRank === 1 ? '🥇' : myRank === 2 ? '🥈' : myRank === 3 ? '🥉' : '';
...
if (isGroup && !isBot && !isFake) {
    try {
        ...
        if (user.daily.dayKey !== getDayKey()) {
            user.daily = { count: 1, dayKey: getDayKey() };
        } else user.daily.count++;

        if (user.weekly.weekKey !== getWeekKey()) {
            user.weekly = { count: 1, weekKey: getWeekKey() };
        } else user.weekly.count++;
...
setInterval(() => {
    for (const gid of groupsToSave) {
        const file = `${RANKING_FOLDER}/${gid}.json`;
        fs.writeFileSync(file, JSON.stringify(rankingCache[gid], null, 2));
        groupsToSave.delete(gid);
    }
}, 60000);